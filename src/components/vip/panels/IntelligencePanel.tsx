...
export default function IntelligencePanel() {
  ...
  const [buying, setBuying] = useState<"certificate" | "stamp" | null>(null);
  ...
  async function buyCredits(kind: "certificate" | "stamp") {
    ...
    try {
      ...
      // Non-fatal — the button just stops spinning.
    }
    setBuying(null);
  }
      ...
      <div className="grid gap-3.5 grid-cols-1 md:grid-cols-[1.6fr_1fr]">
        ...
        <div className="flex flex-col gap-3.5">
          <div className="card elev-sm gap-3 p-5" style={{ background: tokens.surface, border: `1px solid ${tokens.border}` }}>
            ...
            {[
              { kind: "certificate" as const, label: "Certificate credits", value: facts?.certificateCreditsRemaining ?? null, max: CERT_MAX },
              { kind: "stamp" as const, label: "Stamp credits", value: facts?.stampCreditsRemaining ?? null, max: STAMP_MAX },
            ].map((c) => (
